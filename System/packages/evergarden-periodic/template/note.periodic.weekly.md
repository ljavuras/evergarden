...
tp.addTags([
    'note/periodic/weekly',
    'is/personal'
]);

await tp.file.move(`${Periodic.weekly.path}/${tp.file.title}`);
...
```toggl
SUMMARY
FROM <% weekMoment.weekday(0).format('YYYY-MM-DD') %> TO <% weekMoment.weekday(6).format('YYYY-MM-DD') %>
SORT DESC
TITLE "<% weekMoment.format('YYYY [Week] w') %>"
```
